 ...
 * WOIMAI 红包类型的处理
 * ============================================================================
 * * 版权所有 2005-2012 woimai.com，并保留所有权利。
...
/* 初始化$exc对象 */
$exc = new exchange($ecs->table('gift_card_batch'), $db, 'batch_id', 'type_name');
...
{
    /* 检查权限 */
    admin_priv('gift_card');
    //$smarty->assign('ur_here',     $_LANG['04_gift_card_list']);
    $smarty->assign('action_link', array('text' => '添加卡片批次', 'href' => 'gift_card.php?act=add'));
    ...
    assign_query_info();
    $smarty->display('gift_card_batch.htm');
}
...
if ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('gift_card_batch.htm'), '',
...
if ($_REQUEST['act'] == 'remove')
{
    ...
    /* 删除批次下的礼品卡 */
    $db->query("DELETE FROM " .$ecs->table('gift_card'). " WHERE batch_id = '$id'");
    $content = '删除了批次为 '.$id.' 的礼品卡批次及其包含的礼品卡';
    admin_log($content, 'remove', 'gift_card');
...
if ($_REQUEST['act'] == 'add')
{
    ...
    $smarty->assign('ur_here',      '添加卡片批次');
    $smarty->assign('action_link',  array('href' => 'gift_card.php?act=list', 'text' => '批次列表'));
    ...
    assign_query_info();
    $smarty->display('gift_card_batch_info.htm');
}
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    if(!empty($_POST['card_num'])){
        ...
        $reg = '/^\d*$/';
        if(!preg_match($reg,$card_num)){
            $link[] = array('text' => $_LANG['go_back'], 'href' => 'javascript:history.back(-1)');
            sys_msg('您输入的数量不合法', 0, $link);
        }

    }else{
        $link[] = array('text' => $_LANG['go_back'], 'href' => 'javascript:history.back(-1)');
        sys_msg('请输入卡片数量', 0, $link);
    }

    /* 检查金额是否合法 */
    if(!empty($_POST['card_money'])){
        $card_money   = !empty($_POST['card_money']) ? trim($_POST['card_money']) : '';
        $reg = '/^\d*$/';
        if(!preg_match($reg,$card_money)){
            $link[] = array('text' => $_LANG['go_back'], 'href' => 'javascript:history.back(-1)');
            sys_msg('您输入的金额不合法', 0, $link);
            die();
        }

    }else{
        $link[] = array('text' => $_LANG['go_back'], 'href' => 'javascript:history.back(-1)');
        sys_msg('请输入卡片可抵金额', 0, $link);
        die();
    }
   ...
   /* $sql = "SELECT COUNT(*) FROM " .$ecs->table('bonus_type'). " WHERE type_name='$type_name'";
    ...
    $add_time = local_strtotime(date('Y-m-d H:i:s',time()));

    /* 插入数据库。 */
    $sql = "INSERT INTO ".$ecs->table('gift_card_batch')." (card_num, card_money,use_start_date,use_end_date,add_time)
    VALUES ('$card_num',
            '$card_money',
            '$use_startdate',
            '$use_enddate',
    ...
    $sql_id = "select max(batch_id) as id from ".$ecs->table('gift_card_batch');
    $res = $db->getRow($sql_id);
    $content = '添加了批次为'.$res['id'].'的礼品卡';
    /* 记录管理员操作 */
    admin_log($content, 'add', 'gift_card');
    ...
    $link[0]['href'] = 'gift_card.php?act=list';

    $link[1]['text'] = '点击跳转！';
    $link[1]['href'] = 'gift_card.php?act=list';

    sys_msg("添加批次&nbsp;" . $_LANG['attradd_succed'],1, $link);
...
if ($_REQUEST['act'] == 'send')
{
    ...
    admin_priv('gift_card');
    @set_time_limit(0);
    $batch_id =  !empty($_REQUEST['id'])  ? intval($_REQUEST['id'])  : '';
    //判断此批次是否发放过
    $sql = "select * from ".$ecs->table('gift_card')." where batch_id = ".$batch_id;
    $row = $db->getAll($sql);
    if(!empty($row)){
        $link[] = array('text' => $_LANG['go_back'], 'href' => 'javascript:history.back(-1)');
        sys_msg('该批次已生成，不可重复发放', 0, $link);
        die();
    }
    //echo $batch_id;exit;
    //获取此红包生成的数量和可抵金额
    $sql = "select * from ".$ecs->table('gift_card_batch')." where batch_id = ".$batch_id;
    $row = $db->getAll($sql);

    if(!empty($row)){
        //print_r($row);exit;
        $card_num = intval($row[0]['card_num']);
        $use_start_date = intval($row[0]['use_start_date']);
        $use_end_date = intval($row[0]['use_end_date']);
        $card_money = intval($row[0]['card_money']);
        ...
        {
            //$bonus_sn = ($num + $i) . str_pad(mt_rand(0, 9999), 4, '0', STR_PAD_LEFT);//原生成规则
            $bonus_sn = get_unique_id(10);
            $num = $i+1;
            $serial_num = get_serial_num(10,$num,$batch_id);
            $db->query("INSERT INTO ".$ecs->table('gift_card')." (batch_id, serial_num,redeem_code,use_start_date,use_end_date,card_money,remaining_money) VALUES('$batch_id', '$serial_num','$bonus_sn','$use_start_date','$use_end_date','$card_money','$card_money')");

            $j++;
        }

        /* 记录管理员操作 */
        $jilu = '生成了批次为 '.$batch_id.' 的礼品卡';
        admin_log($jilu, 'gift_card_send', 'gift_card');

        /* 清除缓存 */
        clear_cache_files();

        /* 提示信息 */
        $link[0]['text'] = '跳转到礼品卡列表';
        $link[0]['href'] = 'gift_card.php?act=gift_card_list&id='.$batch_id;

        //sys_msg($_LANG['creat_bonus'] . $j . $_LANG['creat_bonus_num'], 0, $link);
        sys_msg('生成成功，跳转中。。。', 0, $link);
    }
...
if ($_REQUEST['act'] == 'activate')
{
    ...
    if(isset($batch_id)){
        //查询是否已经激活
        $sql = "select is_activated from ".$ecs->table('gift_card_batch')." where batch_id =".$batch_id;
        $row = $db->getAll($sql);
        if(!empty($row)){
            if($row[0]['is_activated'] == 2){
                $link[] = array('text' => $_LANG['go_back'], 'href' => 'javascript:history.back(-1)');
                sys_msg('该批次已经激活，不可重复激活', 0, $link);
                die();
            }
        }
        $sql = "update ".$ecs->table('gift_card_batch')." set is_activated = 2 where batch_id =".$batch_id;
        $res = $db->query($sql);
        if($res){
            $jilu = '激活了批次为 '.$batch_id.' 的礼品卡';
            /* 记录管理员操作 */
            admin_log($jilu, 'gift_card_active', 'gift_card');
...
//获取礼品卡列表
if ($_REQUEST['act'] == 'gift_card_list')
{
    /* 检查权限 */
    admin_priv('gift_card');
    $smarty->assign('full_page',    1);
    ...
    $list = get_gift_card_list($batch_id);
    ...
    assign_query_info();
    $smarty->display('gift_card_list.htm');
}
...
//
//        /* 查询此红包类型信息 */
//        $bonus_type = $db->GetRow("SELECT type_id, type_name,use_start_date,use_end_date FROM ".$ecs->table('bonus_type').
//            " WHERE type_id='$_REQUEST[id]'");
//
//        /* 查询红包类型的商品列表 */
...
if ($_REQUEST['act'] == 'send_by_user')
{
    ...
    $use_start_date = $bonus_type['use_start_date'];
    $use_end_date = $bonus_type['use_end_date'];
    ...
    foreach ($user_list AS $key => $val)
    {
        ...
        if (add_to_maillist($val['user_name'], $val['email'], $tpl['template_subject'], $content, $tpl['is_html']))
        {
            ...
            $sql = "INSERT INTO " . $ecs->table('user_bonus') .
                "(bonus_type_id, bonus_sn, user_id,use_start_date,use_end_date, used_time, order_id, emailed) " .
                "VALUES ('$_REQUEST[id]', 0, '$val[user_id]', ".$use_start_date.", ".$use_end_date.",0,0 ," .BONUS_MAIL_SUCCEED. ")";
            $db->query($sql);
        }
        else
        {
            ...
            $sql = "INSERT INTO " . $ecs->table('user_bonus') .
                "(bonus_type_id, bonus_sn, user_id,use_start_date,use_end_date, used_time, order_id, emailed) " .
                "VALUES ('$_REQUEST[id]', 0, '$val[user_id]', ".$use_start_date.", ".$use_end_date.",0,0 ," .BONUS_MAIL_FAIL. ")";
            $db->query($sql);
        }
...
function get_type_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT COUNT(*) FROM ".$GLOBALS['ecs']->table('gift_card_batch');
        ...
        $sql = "SELECT * FROM " .$GLOBALS['ecs']->table('gift_card_batch') ." order by add_time desc";
    ...
    foreach($arr['item'] as $key=>$value){
        $arr['item'][$key]['use_start_time'] = local_date('Y-m-d H:i', $value['use_start_date']);
        $arr['item'][$key]['use_end_time'] = local_date('Y-m-d H:i', $value['use_end_date']);
        $arr['item'][$key]['add_time'] = local_date('Y-m-d H:i', $value['add_time']);
        //判断此礼品卡批次是否已生成礼品卡
        $sql = "SELECT count(*) as num FROM " .$GLOBALS['ecs']->table('gift_card') ." where batch_id =".$value['batch_id'];
        $row = $GLOBALS['db']->getrow($sql);
        if(intval($row['num'])<=0){
            $arr['item'][$key]['is_send']=1;//未生成
        }else{
            $arr['item'][$key]['is_send']=2;//已生成
        }
    }
...
//生成现金卡序列号（新加2017.3.28）
function get_unique_id($length=16)
{
    /*if($pool == "") $pool .= "abcdefghijklmnopqrstuvwxyz0123456789ABCNOPQRSTUVWXYZ
    ";
...
//生成现金卡序列号（新加2017.3.28）
function get_serial_num($length=16,$num,$id=0)
{
    $str=$id;
    $len = $length-strlen($id);
    $need_len = $len -strlen($num);
    for($a=0;$a<$need_len;$a++){
        $str.=0;
    }
    $str.=$num;
    return $str;
...
//获取礼品卡列表
function get_gift_card_list($id=0)
{
    /* 查询条件 */
    /*$filter['sort_by']    = empty($_REQUEST['sort_by']) ? 'bonus_type_id' : trim($_REQUEST['sort_by']);
    $filter['sort_order'] = empty($_REQUEST['sort_order']) ? 'DESC' : trim($_REQUEST['sort_order']);
    ...
    $sql = "SELECT COUNT(*) FROM ".$GLOBALS['ecs']->table('gift_card'). "where batch_id =".$id;
    $filter['record_count'] = $GLOBALS['db']->getOne($sql);

    /* 分页大小 */
    $filter = page_and_size($filter);

    $sql = "select * from " . $GLOBALS['ecs']->table('gift_card') . " where batch_id =".$id;;
    $row = $GLOBALS['db']->getAll($sql);

    foreach($row as $key=>$value){
        $row[$key]['use_start_time'] = local_date('Y-m-d H:i', $value['use_start_date']);
        $row[$key]['use_end_time'] = local_date('Y-m-d H:i', $value['use_end_date']);
        //$row[$key]['add_time'] = local_date('Y-m-d H:i', $value['add_time']);
        $row[$key]['use_time'] = $value['use_time']!=0?local_date('Y-m-d H:i', $value['use_time']):'';
    }
