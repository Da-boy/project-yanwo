...
elseif ($_REQUEST['act'] == 'list')
{
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('order_brief_list.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('order_brief_list.htm'), '', array('filter' => $order_list['filter'], 'page_count' => $order_list['page_count']));
}
...
elseif($_REQUEST['act'] == 'toconfirm'){
    ...
    //修改订单状态
    $sql = "update " . $GLOBALS['ecs']->table('order_info') . " set is_kitchen = 2 where order_sn =".$order_sn;
