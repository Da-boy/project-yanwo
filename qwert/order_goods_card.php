...
{
    /* 检查权限 */
    admin_priv('order_goods_card');

    /* 模板赋值 */
    $smarty->assign('ur_here', $_LANG['02_order_list']);
    $smarty->assign('action_link', array('href' => 'order_goods_card.php?act=order_query', 'text' => $_LANG['03_order_query']));
    ...
    //获取当前是周几
    $w=local_date('w');
    $smarty->assign('now_day',$w);
    /* 显示模板 */
    assign_query_info();
    //print_r($order_list);exit;
    $smarty->display('order_goods_card.htm');
}

//中央大厅每周配送后点击确认
elseif($_REQUEST['act'] == 'toconfirm'){
    $card_id = $_GET['card_id'];
    //查询此套餐卡的信息
    $sql = "select * from ".$GLOBALS['ecs']->table('order_goods_card')." where card_id = ".$card_id;
    $rs = $GLOBALS['db']->getRow($sql);
    if(!empty($rs)){
        $dec_num = intval($rs['week_num']);
        $now_time = mktime()-8*3600;
        //修改套餐剩余量
        $sql = "update " . $GLOBALS['ecs']->table('order_goods_card') . " set remain_num = remain_num-'$dec_num',send_number=send_number+'$dec_num',last_time='$now_time' where card_id =".$card_id;
        $result = $GLOBALS['db']->query($sql);
    }

    if($result){
        $links[] = array('text' => '点击立即返回！', 'href' => 'order_goods_card.php?act=list');
...
//套餐卡信息
elseif($_REQUEST['act'] == 'card_info'){
    admin_priv('order_goods_card_edit');
    $card_id = $_GET['card_id'];
    //查询此套餐卡的信息
    $sql = "select * from ".$GLOBALS['ecs']->table('order_goods_card')." where card_id = ".$card_id;
    $rs = $GLOBALS['db']->getRow($sql);
    if(!empty($rs)){
        //通过订单id获取订单号
        $sql = "select order_sn from ".$GLOBALS['ecs']->table('order_info')." where order_id = ".$rs['order_id'];
        $result = $GLOBALS['db']->getRow($sql);
        if(!empty($result)){
            $rs['order_sn'] = $result['order_sn'];
        }

        $smarty->assign('card_info',   $rs);
    }
    //print_r($rs);exit;
    $smarty->display('order_goods_card_info.htm');
}

//套餐卡发货信息修改
elseif($_REQUEST['act'] == 'newsend'){
    admin_priv('order_goods_card_edit');
    //print_r($_POST);exit;
    ...
    //print_r($day);exit;
    //修改套餐剩余量
    $sql = "update " . $GLOBALS['ecs']->table('order_goods_card') . " set week_num = '$week_num',total_num = '$total_num',send_number = '$send_num',remain_num = '$remain_num',last_time='$now_time' where card_id =".$card_id;
    $result = $GLOBALS['db']->query($sql);
    if($result){
        //获取此套餐卡的订单信息
        //查询此套餐卡的信息
        ...
        /* todo 记录日志 */
        $sn = '订单号'.$order_sn.'下的套餐卡：'.$goods_name;
        admin_log($sn, 'edit', 'order_card');
        $links[] = array('text' => '点击立即返回！', 'href' => 'order_goods_card.php?act=card_info&card_id='.$card_id);
        //$links[] = array('text' => $_LANG['delivery_sn'] . $_LANG['detail'], 'href' => 'order.php?act=delivery_info&delivery_id=' . $delivery_id);
        sys_msg('操作成功', 0, $links);
    }else{
        die(json_encode('2'));
...
elseif($_REQUEST['act'] == 'card_edit'){
    ...
    //print_r($day);exit;
    //修改套餐剩余量
    //$sql = "update " . $GLOBALS['ecs']->table('order_goods_card') . " set week_num = '$week_num',total_num = '$total_num',send_number = '$send_num',remain_num = '$remain_num',last_time='$now_time',day='$day',post_time='$post_time' where card_id =".$card_id;
    $sql = "update " . $GLOBALS['ecs']->table('order_goods_card') . " set day='$day',post_time='$post_time' where card_id =".$card_id;
    $result = $GLOBALS['db']->query($sql);
    if($result){
        //获取此套餐卡的订单信息
        //查询此套餐卡的信息
...
elseif ($_REQUEST['act'] == 'fi_list')
{
    /* 检查权限 */
    admin_priv('order_goods_card_finish');
    ...
    $order_list = order_goods_card_finish();
    $smarty->assign('order_list',   $order_list['orders']);
    $smarty->assign('filter',       $order_list['filter']);
    $smarty->assign('record_count', $order_list['record_count']);
    $smarty->assign('page_count',   $order_list['page_count']);
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('order_goods_card_finish.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('order_goods_card.htm'), '', array('filter' => $order_list['filter'], 'page_count' => $order_list['page_count']));
}
...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    else
    {
        ...
        /* 显示模板 */
        assign_query_info();
        $smarty->display('order_info_brief.htm');
    }
}
...
elseif ($_REQUEST['act'] == 'delivery_ship')
{
    ...
    //通知微信发货
    $sql = "select openid, transid from ".$GLOBALS['ecs']->table('pay_log')." where order_id = ".$order['order_id'];
    $rs = $GLOBALS['db']->getRow($sql);
    //确认是使用微信支付
    if(!empty($rs) && !empty($rs['openid']) && !empty($rs['transid'])){
        define('IN_ECTOUCH', true);
        include(ROOT_PATH . 'mobile/weixin/wxpay_send.php');
    }
    ...
    if(file_exists($file) && $order['user_id'] > 0){
        ...
        if(!empty($remind_title) && !empty($openid)){
            $order_url = $GLOBALS['ecs']->url() . 'mobile/?c=user&a=order_detail&order_id='.$order['order_id'];
...
elseif ($_REQUEST['act'] == 'operate')
{
    ...
    elseif (isset($_POST['print']))
    {
        ...
        foreach ($order_sn_list as $order_sn)
        {
            ...
            {
                if (!admin_priv('order_view_brief', '', false))
                {
                    continue;
                }
            }
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT o.order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
            "o.pay_status, o.consignee, o.address, o.email, o.tel, o.mobile, o.is_kitchen, o.extension_code, o.extension_id, " .
            "(" . order_amount_field('o.') . ") AS total_fee, " .
            "IFNULL(u.user_name, '" .$GLOBALS['_LANG']['anonymous']. "') AS buyer ".
            " FROM " . $GLOBALS['ecs']->table('order_info') . " AS o " .
            " LEFT JOIN " .$GLOBALS['ecs']->table('users'). " AS u ON u.user_id=o.user_id ". $where .
            " ORDER BY  is_kitchen asc,pay_time desc".
    ...
    foreach($row as $key => $value){
        $my_order_id = $value['order_id'];
        $sql = "select goods_name,goods_sn,goods_number,goods_price,goods_attr from ".$GLOBALS['ecs']->table('order_goods')." where order_id=".$my_order_id;
        $order_goods = $GLOBALS['db']->getAll($sql);
        $row[$key]['order_goods'] = $order_goods;
    }
...
function order_goods_card()
{
    ...
    if ($result === false)
    {
        ...
        $filter['composite_status'] = isset($_REQUEST['composite_status']) ? intval($_REQUEST['composite_status']) : -1;
        $filter['group_buy_id'] = isset($_REQUEST['group_buy_id']) ? intval($_REQUEST['group_buy_id']) : 0;
        $filter['day'] = isset($_REQUEST['day']) ? $_REQUEST['day'] : '';
        ...
        //var_dump($filter['day']);exit;
        //按配送日查询
        if(!empty($filter['day'])){

                if($filter['day']=='6'){
                    $where .= " AND  isnull(u.day)";
                }else{
                    $where .= " AND u.day =".$filter['day'];
                }
        ...
        //
        $this_weektime = local_mktime(0,0,0,date("m"),date("d")-date("w")+1,date("Y"));
        $where .= " and u.remain_num > 0";
        /* 分页大小 */
        $filter['page'] = empty($_REQUEST['page']) || (intval($_REQUEST['page']) <= 0) ? 1 : intval($_REQUEST['page']);
        ...
        if ($filter['user_name'])
        {
            //$sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('order_goods_card') . " AS u  " . $where;
            $sql = "SELECT count(*) FROM " . $GLOBALS['ecs']->table('order_goods_card') . " AS u " .
                " inner JOIN " .$GLOBALS['ecs']->table('order_info'). " AS o ON u.order_id=o.order_id ". $where;
        }
        else
        {
            //$sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('order_goods_card') . " AS u ". $where;
        ...
        /* 查询 */
        /*$sql = "SELECT o.order_id, o.order_sn, o.add_time, o.order_amount, o.money_paid," .
            "o.consignee, o.address, o.email, o.tel, o.mobile " .
            " FROM " . $GLOBALS['ecs']->table('order_info') . " AS o " .
            " inner JOIN " .$GLOBALS['ecs']->table('order_goods_card'). " AS u ON u.order_id=o.order_id  ORDER BY u.add_time desc".
            " LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ",$filter[page_size]";*/
        $sql = "SELECT o.order_id, o.order_sn, o.add_time, o.order_amount, o.money_paid," .
            "o.consignee, o.address, o.email, o.tel, o.mobile,u.* " .
            " FROM " . $GLOBALS['ecs']->table('order_goods_card') . " AS u " .
            " left JOIN " .$GLOBALS['ecs']->table('order_info'). " AS o ON u.order_id=o.order_id ". $where ." ORDER BY u.add_time desc".
            " LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ",$filter[page_size]";
    ...
    foreach ($row AS $key => $value)
    {
        ...
        $row[$key]['day'] = $value['day'];
        $row[$key]['last_order_time'] = local_date('Y-m-d H:i', $value['last_time']);
        $row[$key]['post_time'] = $value['post_time'];
        if($value['last_time']<$this_weektime && $value['last_time']!=0){
            $row[$key]['is_send'] = 1;
        }elseif($value['last_time']>$this_weektime){
            $row[$key]['is_send'] = 2;
        }elseif($value['last_time']==0){
            $row[$key]['is_send'] = 3;
        }
        if ($value['order_status'] == OS_INVALID || $value['order_status'] == OS_CANCELED)
        {
...
//function order_goods_card_finish()
//{
//    $result = get_filter();
//    if ($result === false)
//    {
...
//            "o.consignee, o.address, o.email, o.tel, o.mobile " .
//            " FROM " . $GLOBALS['ecs']->table('order_info') . " AS o " .
//            " RIGHT JOIN " .$GLOBALS['ecs']->table('order_goods_card'). " AS u ON u.order_id=o.order_id  ORDER BY u.add_time desc".
//            " LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ",$filter[page_size]";*/
//        $sql = "SELECT o.order_id, o.order_sn, o.add_time, o.order_amount, o.money_paid," .
//            "o.consignee, o.address, o.email, o.tel, o.mobile,u.* " .
//            " FROM " . $GLOBALS['ecs']->table('order_info') . " AS o " .
//            " inner JOIN " .$GLOBALS['ecs']->table('order_goods_card'). " AS u ON u.order_id=o.order_id ".$where." ORDER BY u.last_time desc".
//            " LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ",$filter[page_size]";
//
//        foreach (array('order_sn', 'consignee', 'email', 'address', 'zipcode', 'tel', 'user_name') AS $val)
//        {
...
function curlGet($url, $timeout = 5, $header = "") {
    ...
    $header = empty($header) ? $dheader : $header;
    $ch = curl_init();
