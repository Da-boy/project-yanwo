...
elseif ($_REQUEST['act'] == 'list')
{
    ...
    //获取当前音乐
    $sql =  "select * from ".$GLOBALS['ecs']->table('remind_music')." where m_id = 1";
    $music_info = $GLOBALS['db']->getRow($sql);
    if(!empty($music_info)){
        $smarty->assign('remind_music', $music_info['music_name']);
    }
    /* 显示模板 */
    assign_query_info();
    $smarty->display('order_view_brief.htm');
}
...
{//订单已完成简表
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('order_view_brief_finish.htm');
}
...
elseif($_REQUEST['act'] == 'toconfirm'){
    ...
    //获取此订单下的套餐卡类商品
    $sql1 = "select * from ".$GLOBALS['ecs']->table('order_goods_card')." where order_id=".$order_id;
    $order_goods_card = $GLOBALS['db']->getAll($sql1);
    if(!empty($order_goods_card)){
        $time = mktime()-8*3600;
        foreach($order_goods_card as $key =>$value){
            $sql2 = "update " . $GLOBALS['ecs']->table('order_goods_card') . " set send_number = (send_number+week_num),remain_num=(remain_num-week_num),last_time='$time' where card_id =".$value['card_id'];
            $result = $GLOBALS['db']->query($sql2);
        }
    }
    //修改订单状态
    //$sql = "update " . $GLOBALS['ecs']->table('order_info') . " set is_kitchen = 2 where order_id =".$order_id;
    $sql = "update " . $GLOBALS['ecs']->table('order_info') . " set is_kitchen = 2,order_status=5,shipping_status=1,pay_status=2 where order_id =".$order_id;
    ...
    if($result){
        ...
        if($user_id){
            //由用户获取推荐人id
            $sql2='SELECT parent_id FROM ecs_users  where user_id='.$user_id;
            $user_info = $db->getRow($sql2);
            $parent_id = $user_info['parent_id'];
            if($parent_id != 0){
                //检测是否已经赠送过邀请优惠券
                $sql2 = 'SELECT count(bonus_id) as count FROM ecs_user_bonus where bonus_type_id=2 AND user_id ='.$parent_id.' AND invitee_id='.$user_id;
                $res2 = $db->getRow($sql2);
                if($res2['count']<=0){
                    //  进行优惠券赠送操作
                    $data['user_id'] = $parent_id;
                    $data['bonus_type_id'] = 2;
                    $data['invitee_id']=$user_id;
                    $data['use_start_date']=  strtotime(date("Y-m-d",gmtime())." 00:00:00");
                    $start = strtotime(date("Y-m-d",gmtime())." 00:00:00");
                    $data['use_end_date']=strtotime(date("Y-m-d",gmtime()+3600*24*15)." 23:23:59");
                    $end = strtotime(date("Y-m-d",gmtime()+3600*24*15)." 23:23:59");
                    $sql3 = "insert into ecs_user_bonus(user_id,bonus_type_id,invitee_id,use_start_date,use_end_date) values(".$parent_id.",2,".$user_id.",".$start.",".$end.")";
                    /*for($i=1;$i<=4;$i++){
        ...
        $links[] = array('text' => '点击立即返回！', 'href' => 'order_view_brief.php?act=list');
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('order_view_brief.htm'), '', array('filter' => $order_list['filter'], 'page_count' => $order_list['page_count']));
}
...
elseif ($_REQUEST['act'] == 'operate')
{
    ...
    elseif (isset($_POST['remove']))
    {
        ...
        if (!$batch)
        {
            ...
            $db->query("DELETE FROM ".$ecs->table('order_goods_card'). " WHERE order_id = '$order_id'");
    ...
    elseif (isset($_POST['send']))
    {
        ...
        if ($batch) {
            ...
            foreach ($order_ids as $key => $value) {
                $order = order_info(0,$value);
                //获取此订单下的套餐卡类商品
                $now_order_id = intval($order['order_id']);
                $sql1 = "select * from ".$GLOBALS['ecs']->table('order_goods_card')." where order_id=".$now_order_id;
                $order_goods_card = $GLOBALS['db']->getAll($sql1);
                if(!empty($order_goods_card)){
                    $time = mktime()-8*3600;
                    foreach($order_goods_card as $key =>$val){
                        $sql2 = "update " . $GLOBALS['ecs']->table('order_goods_card') . " set send_number = (send_number+week_num),remain_num=(remain_num-week_num),last_time='$time' where card_id =".$val['card_id'];
                        $result = $GLOBALS['db']->query($sql2);
                    }
                }
                //修改订单状态
                //$sql = "update " . $GLOBALS['ecs']->table('order_info') . " set is_kitchen = 2 where order_id =".$order_id;
                $sql = "update " . $GLOBALS['ecs']->table('order_info') . " set is_kitchen = 2,order_status=5,shipping_status=1 where order_sn =".$value;
                $res = $GLOBALS['db']->query($sql);
                //var_dump($res);exit;

                //发优惠劵（2017.4.18）
                //由订单id获取当前购买人的用户id
                $sql = 'select user_id from ecs_order_info where order_sn =' . $value;
                $order_info = $db->getRow($sql);
                $user_id = $order_info['user_id'];
        ...
        /* 返回 */
        sys_msg('操作成功！正在跳转。。。', 0, array(array('href' => 'order_view_brief.php?act=list', 'text' => '点击立即返回')));
        //print_r($_POST);exit;
    }
    /* 发货单删除 */
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        if ($agency_id > 0)
        {
            ...
            $where .= " and o.shipping_status = 0 and o.pay_status = 2 and o.is_kitchen=1";
        /* 分页大小 */
...
function order_list_finish()
{
    ...
    if ($result === false)
    {
        ...
        $where .= " and shipping_status > 0 and o.pay_status = 2 and o.is_kitchen=2";
        /* 分页大小 */
        $filter['page'] = empty($_REQUEST['page']) || (intval($_REQUEST['page']) <= 0) ? 1 : intval($_REQUEST['page']);
