...
//			}
        ...
        //     cartInfo.innerHTML = result.content;
        //  }
        var number=document.getElementById('total_ge');

        if (number) {
            number.innerHTML = result.cart_number;
 ...
 * 添加商品到购物车 (用于首页列表)
 */
function addToCart_new(goodsId, parentId) {
    var goods = new Object();
    var spec_arr = new Array();
    var fittings_arr = new Array();
    var number = 1;
    ...
    $.post('index.php?m=default&c=flow&a=add_to_cart', {
        ...
        addToCartResponse_new(data,goodsId);
    }, 'json');
    //Ajax.call('index.php?m=default&c=flow&a=add_to_cart', 'goods=' + goods.toJSONString(), addToCartResponse_quick, 'POST', 'JSON');
}

/* *
 * 处理添加商品到购物车的反馈信息(用于首页加入购物车)
 */
function addToCartResponse_new(result,goodsId) {
    if (result.error > 0) {
        // 如果需要缺货登记，跳转
...
//			}
        ...
        //  }
        //页面效果展示
        change_cart_tab(goodsId);
    }
}
/* *
 * 删除商品到购物车 (用于首页列表)
 */
function deleteToCart_new(goodsId , parentId) {
    var goods = new Object();
    var spec_arr = new Array();
    var fittings_arr = new Array();
    var number = 1;
    ...
    goods.number = number;
    goods.parent = (typeof (parentId) == "undefined") ? 0 : parseInt(parentId);
    $.post('index.php?m=default&c=flow&a=delete_cart_goods', {
        goods: $.toJSON(goods)
    }, function(data) {
        deleteToCartResponse_new(data,goodsId);
    }, 'json');
    //Ajax.call('index.php?m=default&c=flow&a=add_to_cart', 'goods=' + goods.toJSONString(), addToCartResponse_quick, 'POST', 'JSON');
}
 ...
 * 处理添加商品到购物车的反馈信息(用于首页加入购物车)
 */
function deleteToCartResponse_new(result,goodsId) {
    console.log(result);
    if (result.error > 0) {
        // 如果需要缺货登记，跳转
...
//			}
        ...
        //  }
        //页面效果展示
        change_cart_tab1(goodsId);
    }

    function get_cart_number_common(){
        $.post('index.php?m=default&c=flow&a=get_cart_number', {
        }, function(data) {
            var number=document.getElementById('total_ge');
            if (number) {
