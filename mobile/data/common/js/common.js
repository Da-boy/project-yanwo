 ...
 * 获得选定的商品属性3
 */
function getSelectedAttributes3(formBuy,num) {
    var spec_arr = new Array();
    var j = 0;
	//console.log(num)
    for (i = 0; i < formBuy.elements.length; i++) {
...
function addToCart_quick(goodsId, parentId) {
    ...
    if (formBuy) {
        ...
        if (formBuy.elements['toname']) {
        	cardid = formBuy.elements['cardid'].value;
        	cardto = formBuy.elements['toname'].value;
        	cardfrom = formBuy.elements['fromname'].value;
        	cardmessage = formBuy.elements['tocontent'].value;
        }
    ...
    goods.cardid=cardid;
    goods.cardto=cardto;
    goods.cardmessage=cardmessage;
    goods.cardfrom=cardfrom;
    goods.goods_id = goodsId;
    goods.number = number;
    goods.parent = (typeof (parentId) == "undefined") ? 0 : parseInt(parentId);
    $.post('index.php?m=default&c=flow&a=add_to_cart', {
 ...
 * 添加商品到购物车 
 */
function addToCart_yw(goodsId, parentId) {	
    var goods = new Object();
    var spec_arr = new Array();
    var fittings_arr = new Array();
    var number = 1;
    var formBuy = document.forms['ECS_FORMBUY146'];
    var quick = 0;
    ...
    if (formBuy) {
        ...
        if (formBuy.elements['number']) {
            ...
            str = getSelectedAttributes3(formBuy,'13');
            spec_arr = str.split(',');
            quick = 1;
            goods.quick = quick;
            goods.spec = spec_arr;
			...
			//addToCartResponse_quick(data);
            	if(formBuy.elements['number147']||formBuy.elements['number148']||formBuy.elements['number149']||formBuy.elements['number150']){
                addToCartResponse_yw(data);
            	}else{addToCartResponse_quick(data);}
            }, 'json');
            
        }
        if(formBuy.elements['number147']){
        	 number = formBuy.elements['number147'].value;
             
             str = getSelectedAttributes3(formBuy,'14');
             spec_arr = str.split(',');
             quick = 1;
             goods.quick = quick;
             goods.spec = spec_arr;
            ...
            $.post('index.php?m=default&c=flow&a=add_to_cart', {
            	...
            	if(formBuy.elements['number148']||formBuy.elements['number149']||formBuy.elements['number150']||formBuy.elements['number152']){
                addToCartResponse_yw(data);
            	}else{addToCartResponse_quick(data);}
            }, 'json');
        	
        }
		 if(formBuy.elements['number148']){
        	 number = formBuy.elements['number148'].value;
             
             str = getSelectedAttributes3(formBuy,'10');
             spec_arr = str.split(',');
             quick = 1;
             goods.quick = quick;
             goods.spec = spec_arr;
            ...
            $.post('index.php?m=default&c=flow&a=add_to_cart', {
            	...
            	if(formBuy.elements['number149']||formBuy.elements['number150']||formBuy.elements['number152']){
                addToCartResponse_yw(data);
            	}else{addToCartResponse_quick(data);}
            }, 'json');
        	
        }
		 if(formBuy.elements['number149']){
        	 number = formBuy.elements['number149'].value;
             
             str = getSelectedAttributes3(formBuy,'11');
             spec_arr = str.split(',');
             quick = 1;
             goods.quick = quick;
             goods.spec = spec_arr;
             ...
             $.post('index.php?m=default&c=flow&a=add_to_cart', {
            	...
            	if(formBuy.elements['number150']||formBuy.elements['number152']){
                addToCartResponse_yw(data);
            	}else{addToCartResponse_quick(data);}
            }, 'json');
        	
        }
		 if(formBuy.elements['number150']){
        	 number = formBuy.elements['number150'].value;
             
             str = getSelectedAttributes3(formBuy,'12');
             spec_arr = str.split(',');
             quick = 1;
             goods.quick = quick;
             goods.spec = spec_arr;
		 ...
		 if(formBuy.elements['number152']){
        	 number = formBuy.elements['number152'].value;
             
             str = getSelectedAttributes3(formBuy,'15');
             spec_arr = str.split(',');
             quick = 1;
             goods.quick = quick;
             goods.spec = spec_arr;
...
/* *
 * 处理添加商品到购物车的反馈信息99燕窝新改wang20160621
 */
function addToCartResponse_yw(result) {
    if (result.error > 0) {
        // 如果需要缺货登记，跳转
...
//			}
        ...
        var cartInfo = document.getElementById('ECS_CARTINFO');
        //var cart_url = 'index.php?m=default&c=flow&a=cart';
//        var cart_url = 'index.php?m=default&c=sell&a=index';
//        if(bool==1)
        	cart_url='index.php?m=default&c=flow&a=checkout';
        if (cartInfo) {
            cartInfo.innerHTML = result.content;
        }
        location.href = cart_url;
