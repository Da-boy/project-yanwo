...
class UserController extends CommonController
{
    ...
    public function index()
    {
      ...
      $this->assign('cart_now_number',$cart_now_number);
        ...
        $small_banner = $this->model->table('touch_ad')
            ->field('ad_code,ad_link')
            ->where('position_id = 5')
            ->select();
        if(!empty($small_banner)){
            $this->assign('small_img',$small_banner[0]['ad_code']);
            $this->assign('small_link',$small_banner[0]['ad_link']);
        }
    ...
    public function bonus()
    {
      ...
      $this->assign('cart_now_number',$cart_now_number);
        // 取得用户可用红包
        $user_bonus = model('Order')->user_bonus_byid($_SESSION ['user_id'],0);
        if (!empty($user_bonus)) {
            foreach ($user_bonus as $key => $val) {
                $user_bonus [$key] ['bonus_money_formated'] =(int)$val ['type_money'];
                $user_bonus[$key]['min_goods_amount']=(int)$val['min_goods_amount'];
        ...
        $this->display('user_bonus_list.dwt');
    }
    /**
     * 红包优惠券
     */
    public function bonusgq()
    {
         //购物车数量
      $cart_now_number=insert_cart_info_number();
      $this->assign('cart_now_number',$cart_now_number);
        // 取得用户可用红包
        $user_bonus = model('Order')->user_bonus_byid($_SESSION ['user_id'],1);
        if (!empty($user_bonus)) {
            foreach ($user_bonus as $key => $val) {
                $user_bonus [$key] ['bonus_money_formated'] =(int)$val ['type_money'];
                $user_bonus[$key]['min_goods_amount']=(int)$val['min_goods_amount'];
        ...
        $this->display('user_bonusgq_list.dwt');
    }
    ...
    {
        // $order_list = model('Users')->get_user_orders(105);
        // var_dump($order_list);
       //  exit();
           //购物车数量
      $cart_now_number=insert_cart_info_number();
    ...
    public function async_order_list()
    {
        if (IS_AJAX) {
            ...
            $order_type=isset($_GET['type']) ? intval($_GET['type']) : 0;
            
            $order_list = model('Users')->get_user_orders($this->user_id, $pay, $limit, $start,$order_type);
    ...
    public function order_detail()
    {
        ...
        foreach ($goods_list as $key => $value) {
            ...
            if($value['goods_id']==145){//添加判断，包含有每日抢鲜燕窝的订单，要判断是否有效
                //获取订单添加日期
                $now_addtime = $order['add_time'];
                $that_time = gmstr2time(date("Y-m-d",$now_addtime)." 00:00:00");
                $start=gmstr2time(date("Y-m-d",$now_addtime)." 10:00:00");
                $end=gmstr2time(date("Y-m-d",$now_addtime)." 20:59:59");
                $start2=gmstr2time(date("Y-m-d",$now_addtime)." 21:00:00");
                $end2=gmstr2time(date("Y-m-d",$now_addtime)." 23:59:59");
                //先获取当前时间戳，判断是哪一波
                //时间在当日十点到当日21点
                if($now_addtime>=$start && $now_addtime<=$end){
                    $order_num=model("Order")->get_order_num_by_one($value['goods_id'],$start,$end);
                    if($order_num<100){
                        $this->assign('can_not_pay', 1);
                        $order['handler'] = '<a class="btn btn-info ect-colorf" type="button" href="javascript:void(0);">已取消</a>';
                    }

                }elseif($now_addtime>$end && $now_addtime<=$end2){//在当日21点到凌晨
                    $tomtime = gmstr2time(date("Y-m-d",$now_addtime)." 00:00:00")+34*3600;
                    $order_num=model("Order")->get_order_num_by_one($value['goods_id'],$end,$tomtime);
                    if($order_num<100){
                        $this->assign('can_not_pay', 1);
                        $order['handler'] = '<a class="btn btn-info ect-colorf" type="button" href="javascript:void(0);">已取消</a>';
                    }

                }elseif($now_addtime>=$that_time && $now_addtime<$start){//从凌晨到十点
                    //获取前一天二十一点的时间戳
                    $ysetime = gmstr2time(date("Y-m-d",$now_addtime)." 00:00:00")-3*3600;
                    $order_num=model("Order")->get_order_num_by_one($value['goods_id'],$ysetime,$start);
                    if($order_num<100){
                        $this->assign('can_not_pay', 1);
                        $order['handler'] = '<a class="btn btn-info ect-colorf" type="button" href="javascript:void(0);">已取消</a>';
                    }
        ...
        if (IS_POST) {
            $address = array(
               ...
               // 'province' => I('post.province', 0, 'intval'),
                ...
                'sign_building'=>I('post.tag')
            );
            
            if (model('Users')->update_address($address)) {
                //Header("HTTP/1.1 303 See Other"); 
                Header("Location:". url('address_list')); 
    ...
    public function service()
    {
        if (IS_POST) {
            $message = array(
				...
				'name' => I('post.name'),
                'msg_telephone' => I('post.msg_telephone'),
    ...
    {
        $this->display('user_jifen_rule.dwt');
    }
    /**
     * 套餐卡
     */
    public function card()
    {
        ...
        if(isset($_SESSION['user_id']) && $_SESSION['user_id'] > 0){
            //获取购物车数量
            $card_row = $this->model->table('order_goods_card')

                ->where('user_id = '.$_SESSION['user_id'].' and remain_num > 0')
                ->order('add_time desc')
                ->select();
            if(!empty($card_row)){
                foreach($card_row as $key=>$value){
                    $card_row[$key]['last_time'] = local_date(C('time_format'), $value['last_time']);
                    $card_row[$key]['remain_num'] = ($value['remain_num']<=0) ? 0:($value['remain_num']);
                    //获取每个套餐卡的订单号
                    $order_info = $this->model->table('order_info')
                        ->field('order_sn')
                        ->where('order_id='.$value['order_id'])
                        ->getOne();
                    $card_row[$key]['order_sn'] = $order_info;
                }
    ...
    public function card_gq()
    {
        ...
        if(isset($_SESSION['user_id']) && $_SESSION['user_id'] > 0){
            ...
            if(!empty($card_row)){
                foreach($card_row as $key=>$value){
                    $card_row[$key]['last_time'] = local_date(C('time_format'), $value['last_time']);
                    $card_row[$key]['add_time'] = local_date(C('time_format'), $value['add_time']);
                    $order_info = $this->model->table('order_info')
                        ->field('order_sn')
                        ->where('order_id='.$value['order_id'])
                        ->getOne();
    ...
    public function money()
    {
        if(isset($_SESSION['user_id']) && $_SESSION['user_id'] > 0){
            ...
            $cart_now_number=insert_cart_info_number();
            $this->assign('cart_now_number',$cart_now_number);
            $money = model('Users')->get_user_money($_SESSION['user_id']);
            if($money){
    ...
    //用户个人资金余额页面
    /*public function user_now_money(){
        if(isset($_SESSION['user_id']) && $_SESSION['user_id'] > 0){
            $money = model('Users')->get_user_money($_SESSION['user_id']);
            if($money){
                $this->assign('money',$money);
    ...
    //现金卡兑换操作
    public function card_exchange_money(){
        // 初始化返回数组
        $result = array(
            'error' => false,
            'message' => '',
        ...
        if(isset($_SESSION['user_id']) && $_SESSION['user_id'] > 0){
            ...
            if($_POST['card_code']){
                ...
                //判断兑换码是否存在
                $sql = "SELECT count(card_id) as count FROM ". $this->model->pre ."gift_card where serial_num = '{$card_num}' and redeem_code = '{$card_code}'";
                $row = $this->model->query($sql);
                //print_r($row);exit;
                if($row[0]['count']<=0){
                    $result = array(
                        'error' => false,
                        'message' => '您输入的兑换码和序列号不匹配',
                    );
                    die(json_encode($result));
                }else{
                    //若存在的话,判断是否使用
                    $sql = "SELECT count(card_id) as count FROM ". $this->model->pre ."gift_card where serial_num = '{$card_num}'and redeem_code= '{$card_code}'and user_id = 0";
                    $row = $this->model->query($sql);
                    if($row[0]['count']<=0){
                        $result = array(
                            'error' => false,
                            'message' => '你输入的兑换码已经使用，不能重复兑换！',
                        );
                        die(json_encode($result));
                    }else{
                        //获取此批次卡片可抵金额
                        $sql_info = "SELECT card_money,use_start_date,use_end_date FROM ". $this->model->pre ."gift_card where serial_num = '{$card_num}'and redeem_code='{$card_code}'";
                        $card_info  = $this->model->query($sql_info);
                       //print_r($card_info);exit;
                        if(!empty($card_info)){
                            //判断是否在使用期限内
                            $use_start_date = intval($card_info[0]['use_start_date']);
                            $use_end_date = intval($card_info[0]['use_end_date']);
                            //当前时间戳
                            $now_time = time();
                            if($use_start_date>$now_time){
                                $result = array(
                                    'error' => false,
                                    'message' => '兑换卡使用时间还未到，请耐心等候！',
                                );
                                die(json_encode($result));
                            }

                            if($use_end_date<$now_time){
                                $result = array(
                                    'error' => false,
                                    'message' => '很遗憾，该兑换卡有效时间已过！',
                                );
                                die(json_encode($result));
                            }
                            $card_money = intval($card_info[0]['card_money']);
                            //给用户充值余额
                            $user_id = $_SESSION['user_id'];
                            $sql1 = "update " . $this->model->pre."users set user_money = user_money+".$card_money." where user_id =".$_SESSION['user_id'];
                            $res1 = $this->model->query($sql1);
                            if($res1){
                                //修改此卡片的状态
                                $sql2 = "update ". $this->model->pre."gift_card set remaining_money=remaining_money-".$card_money.",user_id=".$_SESSION['user_id']." where serial_num = '{$card_num}' and redeem_code='{$card_code}'";
                                $res2 = $this->model->query($sql2);
                                if($res2){
                                    $data_i['user_id'] = $_SESSION['user_id'];
                                    $data_i['user_money'] = $card_money;
                                    $data_i['change_time'] = time();
                                    $data_i['change_desc'] = '礼品卡 '.$card_code.' 兑换';
                                    $data_i['change_type'] = 99;
                                    $this->model->table('account_log')
                                        ->data($data_i)
                                        ->insert();
                                    //存入用户账目日志
                                    $result = array(
                                    ...
                                    //用户余额还原
                                    $sql3 = "update " . $this->model->pre."users set user_money = user_money-".$card_money." where user_id =".$_SESSION['user_id'];
    ...
    //现金卡兑换操作
    public function new_card_exchange_money(){
        // 初始化返回数组
        $result = array(
            'error' => false,
            'message' => '',
        ...
        if(isset($_SESSION['user_id']) && $_SESSION['user_id'] > 0){
            ...
            if($_POST['card_code']){
                ...
                //判断兑换码是否存在
                $sql = "SELECT count(card_id) as count FROM ". $this->model->pre ."gift_card where redeem_code = '{$card_code}'";
                $row = $this->model->query($sql);
                //print_r($row);exit;
                if($row[0]['count']<=0){
                    $result = array(
                    ...
                    //判断兑换码是否已经激活
                    $sql_info = "SELECT batch_id FROM ". $this->model->pre ."gift_card where redeem_code='{$card_code}'";
                    $card_info  = $this->model->query($sql_info);
                    $batch_id = $card_info[0]['batch_id'];
                    $sql_batch_info = "select is_activated  from ". $this->model->pre ."gift_card_batch where batch_id =".$batch_id;
                    $batch_info = $this->model->query($sql_batch_info);
                    if($batch_info[0]['is_activated']<=1){
                        $result = array(
                            'error' => false,
                            'message' => '您的卡片尚未激活，请耐心等候！',
                        );
                        die(json_encode($result));
                    }
                    //若存在的话,判断是否使用
                    $sql = "SELECT count(card_id) as count FROM ". $this->model->pre ."gift_card where redeem_code= '{$card_code}'and user_id = 0";
                    $row = $this->model->query($sql);
                    if($row[0]['count']<=0){
                        $result = array(
                            'error' => false,
                        ...
                        //获取此批次卡片可抵金额
                        $sql_info = "SELECT card_money,use_start_date,use_end_date FROM ". $this->model->pre ."gift_card where redeem_code='{$card_code}'";
                        $card_info  = $this->model->query($sql_info);
                        //print_r($card_info);exit;
                        if(!empty($card_info)){
                            //判断是否在使用期限内
                            ...
                            if($res1){
                                //修改此卡片的状态
                                $sql2 = "update ". $this->model->pre."gift_card set remaining_money=remaining_money-".$card_money.",user_id=".$_SESSION['user_id'].",use_time ='{$now_time}' where  redeem_code='{$card_code}'";
                                $res2 = $this->model->query($sql2);
                                if($res2){
                                    $data_i['user_id'] = $_SESSION['user_id'];
                                    $data_i['user_money'] = $card_money;
