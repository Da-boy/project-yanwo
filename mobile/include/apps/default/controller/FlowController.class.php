...
class FlowController extends CommonController {
    ...
    public function add_to_cart() {
        ...
        if (empty($_POST ['goods'])) {
			...
			die(ecs_header("Location:./\n"));
        }
        $json = new EcsJson;
        $goods = $json->decode($_POST ['goods']);
//                //冰糖燕窝抢购判断
//        if($goods->goods_id == 145){
//             // 判断是否已超过抢购时间
//             /*$start_time=strtotime(date("Y-m-d",time())." 10:00:00");
//             $end_time=strtotime(date('Y-m-d',time())." 12:00:00");
//             $now_time=time();
//             if($now_time<$start_time){
//               $result ['error'] = 8;
//              $result ['message'] = "该抢购活动未开始！";
...
//                $result ['error'] = 8;
//              $result ['message'] = "该抢购活动已结束！";
//               $result ['goods_id'] = $goods->goods_id;
//                $result ['parent'] = $goods->parent;
//              die(json_encode($result));
//             }*/
//            //判断是否已超过抢购数量
//             $start=gmstr2time(date("Y-m-d",time())." 00:00:00");
//             $end=gmstr2time(date("Y-m-d",time())." 23:59:59");
//             $order_num=model("Order")->get_order_num_by_one($goods->goods_id,$start,$end);
//            if($order_num>=100){
//              $result ['error'] = 10;
//              $result ['message'] = "该商品已经抢完！";
//               $result ['goods_id'] = $goods->goods_id;
//                $result ['parent'] = $goods->parent;
//              die(json_encode($result));
//            }
//             // 判断是否当日已经抢购过
//            if($_SESSION['user_id']){
//            $re=model("Order")->get_order_by_user_time($_SESSION['user_id'],$goods->goods_id,$start,$end);
//            if(!empty($re)){
//              $result ['error'] = 9;
//              $result ['message'] = "已经抢购过！";
//              $result ['goods_id'] = $goods->goods_id;
//              $result ['parent'] = $goods->parent;
//              die(json_encode($result));
//            }
//            }
//           // 判断是否已加入购物车
//           $cartList=model("Flow")->get_cart_goods_one($goods->goods_id);
//           if(!empty($cartList)){
//              $result ['error'] = 7;
//              $result ['message'] = "该商品已经加入购物车中！";
//              $result ['goods_id'] = $goods->goods_id;
//              $result ['parent'] = $goods->parent;
//              die(json_encode($result));
//           }
//        }
        ...
        if($goods->goods_id == 145){
            ...
            //判断是否已超过抢购数量
            $start=gmstr2time(date("Y-m-d",time())." 10:00:00");
            $end=gmstr2time(date("Y-m-d",time())." 20:59:59");
            $start2=gmstr2time(date("Y-m-d",time())." 21:00:00");
            $end2=gmstr2time(date("Y-m-d",time())." 23:59:59");
            $start1 = gmstr2time(date("Y-m-d",time())." 00:00:00");
            //先获取当前时间戳，判断是哪一波
            $nowtime = gmstr2time(date("Y-m-d H:i:s",time()));
            //时间在当日十点到当日21点
            if($nowtime>=$start && $nowtime<=$end){
                $order_num=model("Order")->get_order_num_by_one($goods->goods_id,$start,$end);
                if($order_num>=100){
                    $result ['error'] = 10;
                    $result ['message'] = "该商品已经抢完,请晚上九点再来吧！";
                    $result ['goods_id'] = $goods->goods_id;
                    $result ['parent'] = $goods->parent;
                    die(json_encode($result));
                }
                ...
                if($_SESSION['user_id']){
                    ...
                    if(!empty($re)){
                        $result ['error'] = 9;
                        $result ['message'] = "今日抢购次数已用完，明日再来哟~~";
                        $result ['goods_id'] = $goods->goods_id;
                        $result ['parent'] = $goods->parent;
                        die(json_encode($result));
                    }
            ...
            }elseif($nowtime>$end && $nowtime<=$end2){//在当日21点到凌晨
                $order_num=model("Order")->get_order_num_by_one($goods->goods_id,$end,$end2);
                if($order_num>=100){
                    $result ['error'] = 10;
                    $result ['message'] = "该商品已经抢完,请明天早上九点再来吧！";
                    $result ['goods_id'] = $goods->goods_id;
                    $result ['parent'] = $goods->parent;
                    die(json_encode($result));
                }
                // 判断是否当日已经抢购过
                if($_SESSION['user_id']){
                    $re=model("Order")->get_order_by_user_time($_SESSION['user_id'],$goods->goods_id,$end,$end2);
                    if(!empty($re)){
                        $result ['error'] = 9;
                        $result ['message'] = "今日抢购次数已用完，明日再来哟~~";
                        $result ['goods_id'] = $goods->goods_id;
            ...
            }elseif($nowtime>=$start1 && $nowtime<$start){//从凌晨到十点
                //获取前一天二十一点的时间戳
                $ysetime = gmstr2time(date("Y-m-d",strtotime("-1 day"))." 21:00:00");
                $order_num=model("Order")->get_order_num_by_one($goods->goods_id,$ysetime,$start);
                if($order_num>=100){
                    $result ['error'] = 10;
                    $result ['message'] = "该批量商品已经抢完,请十点以后点再来吧！";
                    $result ['goods_id'] = $goods->goods_id;
                    $result ['parent'] = $goods->parent;
                    die(json_encode($result));
                }
                // 判断是否当日已经抢购过
                if($_SESSION['user_id']){
                    $re=model("Order")->get_order_by_user_time($_SESSION['user_id'],$goods->goods_id,$ysetime,$start);
                    if(!empty($re)){
                        $result ['error'] = 9;
                        $result ['message'] = "今日抢购次数已用完，明日再来哟~~";
                        $result ['goods_id'] = $goods->goods_id;
        ...
        $order = model('Order')->get_order_by_goods_user($user_id,$goods->goods_id);
        if(!empty($order)){
              $result ['error'] =9;
              $result ['message'] = "已经抢购过！"; 
              $result ['goods_id'] = $goods->goods_id;
        ...
        //20160515更新，如果是冰糖燕窝抢先购，先清空购物车
//        if($goods->goods_id==145||$goods->goods_id==151||$goods->goods_id==153) {
//            model('Order')->clear_cart();
//        }else { 
     //       model('Order')->delete_cart_goods(145);
//            model('Order')->delete_cart_goods(151);
//			model('Order')->delete_cart_goods(153);
//        }
//        if($goods->goods_id==151){
//            $_SESSION['order_type']=2;
//            $_SESSION['card_id']=$goods->cardid;
//            $_SESSION['card_to']=$goods->cardto;
//            $_SESSION['card_message']=$goods->cardmessage;
//            $_SESSION['card_from']=$goods->cardfrom;
//        }
//        switch ($goods->goods_id){
//            case 146:
//                model('Order')->delete_cart_goods(146);
//                break;
//            case 147:
//                model('Order')->delete_cart_goods(147);
//                break;
//            case 148:
//                model('Order')->delete_cart_goods(148);
//                break;
//            case 149:
//                model('Order')->delete_cart_goods(149);
//                break;
//            case 150:
//                model('Order')->delete_cart_goods(150);
//                break;
//            case 152:
//                model('Order')->delete_cart_goods(152);
//                break;
//            default:
//                break;
//        }
       ...
       // $goodsnumber = model('Users')->get_goods_number($goods->goods_id);
        //$goodsnumber=$goodsnumber+1; 
        $goodsnumber=$goods->number; //20160622修改，燕窝99
        //var_dump($goodsnumber,$arrGoods ['extension_code']);
        if (intval(C('use_storage')) > 0 && $arrGoods ['extension_code'] != 'package_buy') {
            if ($arrGoods ['goods_number'] < $goodsnumber) {
                $result['error'] = 1;
        ...
        }else if($goods->number==0){  //20160622修改，燕窝99
             model('Order')->delete_cart_goods($goods->goods_id);
        }
        else {
            // 更新：添加到购物车
    ...
    /**
     * 删除购物车中的商品99燕窝20160621
     */
    public function delete_goods($goodsid) {        
        //删除购物车中的商品
        model('Flow')->flow_drop_cart_goods($goodsid);
...
//        }
//        /* 检查购物车中是否有商品 */
//        $condition = "session_id = '" . SESS_ID . "' " . "AND parent_id = 0 AND is_gift = 0 AND check_status=0 AND rec_type = '$flow_type'";
...
//        //判断购物车中是否有抢购产品，是否可以进行抢购
//        //冰糖燕窝抢购判断
//        $idData=array();
//        foreach($cart_goods as $val){
//            $idData[]=$val['goods_id'];
//            if($val['goods_id']==145){
//                $content=$val['goods_name'];
//            }
//        }
//
//        if(in_array(145,$idData)){
//             // 判断是否已超过抢购时间
//             /*$start_time=strtotime(date("Y-m-d",time())." 10:00:00");
//             $end_time=strtotime(date('Y-m-d',time())." 12:00:00");
//             $now_time=time();
//             if($now_time<$start_time){
//               show_message($content."抢购活动未开始，无法订购该商品", '', url('flow/cart'), 'warning');
//            }
//             if($now_time>$end_time){
//               show_message($content."抢购活动已结束，无法订购该商品", '',  url('flow/cart'), 'warning');
//             }*/
//            //判断是否已超过抢购数量
//             $start=gmstr2time(date("Y-m-d",time())." 00:00:00");
//             $end=gmstr2time(date("Y-m-d",time())." 23:59:59");
//             $order_num=model("Order")->get_order_num_by_one(145,$start,$end);
//            if($order_num>=100){
//                show_message("抢购商品".$content."已抢完，无法订购该商品", '',  url('flow/cart'), 'warning');
//            }
//             // 判断是否当日已经抢购过
//            $re=model("Order")->get_order_by_user_time($_SESSION['user_id'],145,$start,$end);
//            if(!empty($re)){
//                 show_message($content."您已订购过，无法订购该商品", '', url('flow/cart'), 'warning');
//            }
//        }
//        /* 对是否允许修改购物车赋值 */
...
//            // 取得用户可用红包中最先过期的红包
//            $user_bonus = model('Order')->user_bonus_last($_SESSION ['user_id'], $total ['goods_price']);
//            if(!empty($user_bonus)){
//                //使用红包
//                $order['bonus_id'] = $user_bonus['bonus_id'];
//                /* 计算订单的费用 */
//                $total = model('Users')->order_fee($order, $cart_goods, $consignee);
//                $this->assign('total', $total);
//
//                $this->assign('bonus', $user_bonus);
//                $this->assign('allow_use_bonus', 1);
//                //获取用户红包列表
//                $user_bonus_list = model('Order')->user_bonus($_SESSION ['user_id'], $total ['goods_price']);
//                            if (!empty($user_bonus_list)) {
//                                foreach ($user_bonus_list as $key => $val) {
//                                    $user_bonus_list [$key] ['bonus_money_formated'] =(int) $val ['type_money'];
//                                    $user_bonus_list[$key]['min_goods_amount']=(int)$val['min_goods_amount'];
//                                    $user_bonus_list[$key]['use_end_date']=date('Y.m.d',$val ['use_end_date']);
//                                }
//                                $this->assign('bonus_list', $user_bonus_list);
//                            }
//            }else {
//                $this->assign('allow_use_bonus', 0);
...
//    }
    ...
    public function checkout() {
        ...
        if(in_array(145,$idData)){
            ...
            //先获取当前时间戳，判断是哪一波
            $nowtime = gmstr2time(date("Y-m-d H:i:s",time()));
            //echo $start.'--'.$nowtime.'--'.$end.'--'.$end2.'--'.$start1;exit;
            //时间在当日十点到当日21点
            if($nowtime>=$start && $nowtime<=$end){
                $order_num=model("Order")->get_order_num_by_one(145,$start,$end);
                if($order_num>=100){
            ...
            }elseif($nowtime>=$end && $nowtime<=$end){//在当日21点到凌晨
                exit('222');
                $order_num=model("Order")->get_order_num_by_one(145,$end,$end2);
                if($order_num>=100){
                    show_message("抢购商品".$content."已抢完，无法订购该商品", '',  url('flow/cart'), 'warning');
                }
                // 判断是否当批次已经抢购过
                if($_SESSION['user_id']){
                    $re=model("Order")->get_order_by_user_time($_SESSION['user_id'],145,$end,$end2);
                    if(!empty($re)){
                        show_message($content."您已订购过，无法订购该商品", '', url('flow/cart'), 'warning');
                    }
                }
                ...
                //获取前一天二十一点的时间戳
                $ysetime = gmstr2time(date("Y-m-d",strtotime("-1 day"))." 21:00:00");
                $order_num=model("Order")->get_order_num_by_one(145,$start1,$start);
                if($order_num>=100){
                    show_message("抢购商品".$content."已抢完，无法订购该商品", '',  url('flow/cart'), 'warning');
                }
                // 判断是否当批次已经抢购过
                if($_SESSION['user_id']){
                    $re=model("Order")->get_order_by_user_time($_SESSION['user_id'],145,$start1,$start);
                    if(!empty($re)){
                        show_message($content."您已订购过，无法订购该商品", '', url('flow/cart'), 'warning');
                    }
                }
        ...
        /* 如果使用余额，取得用户余额 */
        $use_surplus = C('use_surplus');
        $now_amount = $total['amount'];
        if ((!isset($use_surplus) || C('use_surplus') == '1') && $_SESSION ['user_id'] > 0 && $user_info ['user_money'] >= $now_amount) {//原来是只有>
            // 能使用余额
            $this->assign('allow_use_surplus', 1);
            $this->assign('your_surplus', $user_info ['user_money']);
        }
    ...
    public function consignee() {
        ...
        if ($_SERVER ['REQUEST_METHOD'] == 'GET') {
            ...
            foreach ($consignee_list as $region_id => $consignee) {
                ...
                $city_list [$region_id] = model('RegionBase')->get_regions(2, 27);
			...
			//$this->assign('city_list', model('RegionBase')->get_regions(2, 27));
            ...
            $consignee = array(
                ...
                'sign_building'=>empty($_POST['tag'])?'':I('post.tag')
            );
...
//            'card_id' => isset($_SESSION ['card_id']) ? intval($_SESSION ['card_id']) : 0,
...
//            'mobile_order' => 1,
//            'mobile_pay' => 1,
//            'order_type'=>isset($_SESSION ['order_type']) ? intval($_SESSION ['order_type']) : 1,
//            'card_to'=>trim($_SESSION ['card_to']),
//            'card_from'=>trim($_SESSION ['card_from'])
//        );
//
//        /* 扩展信息 */
...
//
//        /* 插入订单商品 */
//        $sql = "INSERT INTO " . $this->model->pre . "order_goods( " . "order_id, goods_id, goods_name, goods_sn, product_id, goods_number, market_price, " . "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id) " . " SELECT '$new_order_id', goods_id, goods_name, goods_sn, product_id, goods_number, market_price, " . "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id" . " FROM " . $this->model->pre . "cart WHERE session_id = '" . SESS_ID . "' AND check_status = 0 AND rec_type = '$flow_type'";
//        $this->model->query($sql);
//        /* 修改拍卖活动状态 */
...
//        //修改下订单购物车清空问题（新加）
//        $sql_new = "SELECT goods_id FROM " . $this->model->pre . "order_goods WHERE order_id =".$order ['order_id'];
//        $goods_new_order = $this->model->query($sql_new);
//        if(!empty($goods_new_order)){
//            foreach($goods_new_order as $key => $value){
//                $gid = intval($value['goods_id']);
//                $res = model('Order')->delete_cart_goods($gid);
//            }
//        }
//
...
//    }
    ...
    public function done() {
        ...
        $oos = L('oos.' . $how_oos);
        /*if($_POST['time']){
            $end_time = date('H:i',(strtotime($_POST['time'])+3*3600));
        }*/
        // 订单信息
        $order = array(
            'shipping_id' => I('post.shipping'),
            ...
            'card_to'=>trim($_SESSION ['card_to']),
            'card_from'=>trim($_SESSION ['card_from']),
            //'post_time_start'=>$_POST ['time'] ? ($_POST ['time']) : 0,
            //'post_time_end'=>isset($end_time) ? ($end_time) : 0
        );
        /* 扩展信息 */
        if (isset($_SESSION ['flow_type']) && intval($_SESSION ['flow_type']) != CART_GENERAL_GOODS) {
            $order ['extension_code'] = $_SESSION ['extension_code'];
        ...
        if ($order ['order_amount'] >0) {
                ...
                $this->assign('pay_online', $pay_online);
                $this->assign('pay_id',$order ['pay_id']);//新加--判断是什么支付方式（2017.4.5）
    ...
    /*
     * 设置配送地址为默认（新写 2017-03-23）
     */
    public function change_address(){
       $result = array('error' => '0', 'content' => '设置成功！');
        $address_id = intval($_REQUEST['address']);
        if (model('Users')->save_consignee_default($address_id)) {
            die(json_encode($result));
        } else {
    ...
    /*
     * 取消配送地址默认状态（2017-03-24）
     */
    public function cancel_address_default(){
        $result = array('error' => '0', 'content' => '取消成功！');
        $address_id = 0;
        if (model('Users')->cancel_address_default($address_id)) {
            die(json_encode($result));
        } else {
            $result['error'] = '1';
            $result['content'] = '取消失败！';
    ...
    public function consignee_list() {
        if (IS_AJAX) {
            ...
            if ($consignee_list) {
                ...
                $sayList [] = array(
                    'single_item' => "<div  class=\"user_dizhi_content\"><div class=\"user_dizhi_content_img\"><img src=\"/mobile/themes/99yanwo/image/user/dizhi_no.png\" /></div><p class=\"user_dizhi_content_img_p\">您还没有收货地址,请先添加！ </p></div>	"
                );
   ...
   public function  delete_cart_goods(){
        ...
        if (!empty($goods->goods_id)) {
            $id = $goods->goods_id;
            $goods_data = model('Flow')->get_cart_goods_one($id);
            if(!empty($goods_data)){
                if($goods_data['goods_number']-1<=0){
                    //删除购物车中该商品
                    $re=model('Flow')->flow_drop_cart_goods($goods_data['rec_id']);
                       $result ['content'] = insert_cart_info();
                       $result ['cart_number'] = insert_cart_info_number();
                        die(json_encode($result)); 
                }else{
                     //更新购物车中该商品数量
                    if(model('Flow')->update_cart_goods_number($goods_data['rec_id'],1)){
                      $result ['content'] = insert_cart_info();
                      $result ['cart_number'] = insert_cart_info_number();
                      die(json_encode($result));    
                    }else{
               ...
               $result ['error'] = 1;
               $result ['message'] = "传参错误3";
               die(json_encode($result));    
            }
        }else{
          $result ['error'] = 1;
          $result ['message'] = "传参错误2";
          die(json_encode($result));  
        } 
   }
   /*
   ...
   public function change_status(){
       $result ['status'] = false;
        ...
        $re=model('Flow')->cart_goods_status($rec_id,$type);
        if($re){
            $result['status']=true;
        }else{
            $result['status']=false;
   ...
   public function change_status_all(){
        $result ['status'] = false;
        $type=intval($_GET ['type']);
        $re=model('Flow')->cart_goods_status_all($type);
        if($re){
            $result['status']=true;
        }else{
            $result['status']=false;
    ...
    public function test(){
       ...
       /*$time =  local_date('Y-m-d H:i:s','1491760800');
       $time1 =  date("Y-m-d H:i:s",time());*/
        /*$now_order_id = model('Base')->model->table('order_info')
            ->where('order_sn = 2016061797627')
            ->select();

        $start = date("Y-m-d H:i:s",local_mktime(0,0,0,date("m"),date("d")-date("w")+1,date("Y")));
        print_r($now_order_id[0]);
        $reg = '/([a-zA-Z]|\d)+/';
        $str = '123eefgea4e';
        $res = preg_match($reg,$str);
        var_dump($res);exit;*/
        $a = '16:00';
        $b=strtotime($a)+3*3600;
        $c = date('H:i',$b);
        echo $c;exit;
