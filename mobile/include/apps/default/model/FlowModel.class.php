...
class FlowModel extends BaseModel {
  ...
  /*
     * 获取购物车中单个商品信息
     */
    function  get_cart_goods_one($id, $type = CART_GENERAL_GOODS){
        /* 检查该商品是否已经存在在购物车中 */
        $sql = "SELECT rec_id,user_id,session_id,goods_id ,goods_number  FROM " . $this->pre .
                "cart WHERE session_id = '" . SESS_ID . "' AND goods_id = '$id' " .
                "AND parent_id = 0 AND rec_type = '$type'";
        $res = $this->row($sql);
        return $res;
    }
    /*
     * 购物车增加或减少单个商品的数量(0为加一，1为减一)
     */
    public  function update_cart_goods_number($rec_id,$type){
        if($type==0){
           $sql = "UPDATE " . $this->pre . "cart SET goods_number = goods_number+1 WHERE rec_id = '" . $rec_id."'" ;  
        }else{
           $sql = "UPDATE " . $this->pre . "cart SET goods_number = goods_number-1 WHERE rec_id = '" . $rec_id ."'";  
        }
           $re=$this->query($sql); 
     ...
     * 购物车商品选择状态变更
     */
    public  function cart_goods_status($rec_id,$type){       
           $sql = "UPDATE " . $this->pre . "cart SET check_status = ".$type." WHERE rec_id = '" . $rec_id."'" ;  
           $re=$this->query($sql); 
           return $re;
    }
    /*
     * 购物车所有商品选择状态变更
     */
    public  function  cart_goods_status_all($type){
          $sql = "UPDATE " . $this->pre . "cart SET check_status = ".$type." WHERE session_id = '" . SESS_ID."'" ; 
           $re=$this->query($sql); 
           return $re;
    }
}
