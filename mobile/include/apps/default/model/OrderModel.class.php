...
class OrderModel extends BaseModel {
    ...
    function order_info($order_id, $order_sn = '') {
        ...
        if ($order) {
            ...
            //$order['formated_add_time'] = local_date(C('time_format'), $order['add_time']+8*3600);
    ...
    function cart_goods($type = CART_GENERAL_GOODS) {
        $sql = "SELECT c.rec_id, c.user_id, c.goods_id, c.goods_name, c.goods_sn, c.goods_number, " .
                ...
                "c.goods_price * c.goods_number AS subtotal, g.original_img " .
                "FROM " . $this->pre .
                "cart c left join ".$this->pre."goods g on c.goods_id=g.goods_id WHERE c.session_id = '" . SESS_ID . "' " .
                "AND c.rec_type = '$type' AND c.check_status=0";
    ...
    function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0) {
        ...
        if ($num > 0) {
            ...
            if ($row) { //如果购物车已经有此物品，则更新
                $num += $row['goods_number'];//20160622 燕窝修改
                if (model('GoodsBase')->is_spec($spec) && !empty($prod)) {
    ...
    function user_bonus($user_id, $goods_amount = 0) {
        ...
        $sql = "SELECT t.type_id, t.type_name, t.type_money,t.min_goods_amount,b.use_end_date, b.bonus_id " .
                "FROM " . $this->pre . "bonus_type AS t," .
                $this->pre . "user_bonus AS b " .
                "WHERE t.type_id = b.bonus_type_id " .
                "AND b.use_start_date <= '$today' " .
                "AND b.use_end_date >= '$today' " .
     ...
     * @return  array   红包数组
     */
    function user_bonus_last($user_id, $goods_amount = 0) {
        $day = getdate();
        $today = local_mktime(23, 59, 59, $day['mon'], $day['mday'], $day['year']);
        ...
        $sql = "SELECT t.type_id, t.type_name, t.type_money, b.bonus_id " .
            ...
            "AND b.user_id = '$user_id' " .
            "AND b.order_id = 0 ".
            "ORDER BY b.bonus_id ASC limit 1";
        //var_dump($sql);
        return $this->row($sql);
    }
     ...
     * 取得用户当前可用红包
     * @param   int     $user_id        用户id
     * @param   int   $ifpast   是否过期 0 未过期，1已过期
     * @return  array   红包数组
     */
    function user_bonus_byid($user_id, $ifpast = 0) {
        $day = getdate();
        $today = local_mktime(23, 59, 59, $day['mon'], $day['mday'], $day['year']);
        ...
        $sql = "SELECT t.type_id, t.type_name, t.type_money,t.min_goods_amount,b.use_end_date, b.bonus_id " .
            ...
            "AND b.use_start_date <= '$today' " .
            "AND b.user_id <> 0 " .
            "AND b.bonus_sn = 0 " .//新加的一句--判断不显示线下红包即可用作余额的红包（2017.3.28）
            "AND b.user_id = '$user_id' " .
            "AND b.order_id = 0 ";
        if($ifpast)
            $sql.= "AND b.use_end_date <= '$today' " ;
          else 
              $sql.= "AND b.use_end_date >= '$today' " ;
        //var_dump($sql);
        return $this->query($sql);
    ...
    function bonus_info($bonus_id, $bonus_sn = '') {
        $sql = "SELECT t.type_id, t.type_name, t.type_money,t.min_goods_amount, b.* " .
                "FROM " . $this->pre . "bonus_type AS t," .
                $this->pre . "user_bonus AS b " .
                "WHERE t.type_id = b.bonus_type_id ";
        if ($bonus_id > 0) {
    ...
    function use_bonus($bonus_id, $order_id) {
        //判断所使用的红包是否为赠送的红包，若使用赠送的红包则反馈给赠送者同样面值红包20160725新增
        $sql = "SELECT * FROM " . $this->pre .
        "user_bonus WHERE bonus_id = '$bonus_id'";
        $res = $this->row($sql);
        if($res['fuserid'] > 0){
            $data['bonus_type_id'] = $res['bonus_type_id'];//测试红包，需要手动更改
            $data['bonus_sn'] = 0;
            $data['user_id'] = $res['fuserid'];
            $data['used_time'] = 0;
            $data['order_id'] = 0;
            $data['emailed'] = 0; 
...
/**
     ...
     * @return type
     */
    function get_order_thumb_all($order_id) {

        $arr = $this->model->query("SELECT g.goods_thumb FROM " . $this->model->pre . "order_goods as og left join " . $this->model->pre . "goods g on og.goods_id = g.goods_id WHERE og.order_id = " . $order_id );
        return $arr;
    }
    /*
     * 获取某用户已购买的某个产品订单信息
     */
    function get_order_by_goods_user($userid,$goods_id){
        $arr=$this->model->query("select oi.order_sn AS order_sn,oi.order_id AS order_id,oi.user_id AS user_id,oi.pay_status AS pay_status,og.goods_id AS goods_id from ".$this->model->pre."order_info as oi left join ".$this->model->pre."order_goods og on og.order_id = oi.order_id where oi.user_id=".$userid." and og.goods_id=".$goods_id.";");
        return $arr;
    }
     ...
     * 获取某个商品订购数量（某时间段）
     */
    function  get_order_num_by_one($goods_id,$starttime,$endtime){
        $arr=$this->model->query("select count(og.order_id)as num from ".$this->model->pre."order_goods as og  left join ".$this->model->pre."order_info as oi on oi.order_id = og.order_id where oi.pay_status=2 AND oi.add_time>".$starttime." AND oi.add_time<".$endtime." AND og.goods_id=".$goods_id.";");
        return $arr[0]['num'];
    }
     /*
     * 获取某用户已购买的某个产品订单信息数量（在某时间段内）
     */
    function get_order_by_user_time($userid,$goods_id,$starttime,$endtime){
        $arr=$this->model->query("select oi.order_sn AS order_sn,oi.order_id AS order_id,oi.user_id AS user_id,oi.pay_status AS pay_status,og.goods_id AS goods_id from ".$this->model->pre."order_info as oi left join ".$this->model->pre."order_goods og on og.order_id = oi.order_id where oi.user_id=".$userid." and og.goods_id=".$goods_id." AND oi.add_time>".$starttime." AND oi.add_time<".$endtime." AND oi.order_status NOT IN (2,3,4);");
        return $arr;
    }
}
