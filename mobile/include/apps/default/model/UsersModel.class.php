...
class UsersModel extends BaseModel {
    ...
    function update_user_info() {
        ...
        /* 查询会员信息 */
        $time = date('Y-m-d');
        $sql = 'SELECT u.user_money,u.email, u.pay_points, u.user_rank, u.rank_points,u.city, ' .
     ...
     * @return  void
     */
    function update_user_city($city) {
        if (!$_SESSION['user_id']) {
            return false;
        }
        if(!empty($city))
    ...
    function register($username, $password, $email, $other = array()) {
        ...
        if (!ECTouch::user()->add_user($username, $password, $email)) {
             ...
             * 新用户赠送优惠券
             */
         //    $this->add_user_bonus($_SESSION['user_id']);
            /* 推荐处理 */
     ...
     */
    function get_user_orders($user_id, $pay = 1, $num = 10, $start = 0,$order_type=0) {
        /* 取得订单列表 */
        $arr = array();
        ...
        if ($order_type == 0) {
            ...
            $order_type = 'and order_type = ' . $order_type;
        }

        $sql = "SELECT order_id, order_sn, shipping_id, order_status, shipping_status, pay_status, add_time, " .
                "(goods_amount + shipping_fee + insure_fee + pay_fee + pack_fee + card_fee + tax - discount) AS total_fee " .
                " FROM " . $this->pre .
                "order_info WHERE user_id = '$user_id' " . $pay . $order_type." ORDER BY add_time DESC LIMIT $start , $num";
        ...
        foreach ($res as $key => $value) {
            ...
            $value['order_status1'] = L('os.' . $value['order_status']);
            $value['order_status2'] = L('ps.' . $value['pay_status']);
            $value['order_status3'] = L('ss.' . $value['shipping_status']);
            $value['order_status'] = L('os.' . $value['order_status']) . ',' . L('ps.' . $value['pay_status']) . ',' . L('ss.' . $value['shipping_status']);
            //获取订单列表所有商品图片
            $img_temp=model('Order')->get_order_thumb_all($value['order_id']);
            $img_arr=array();
            if(!empty($img_temp)){
               foreach($img_temp as $val){
                $img_arr[]=get_image_path(0, $val['goods_thumb']);
            }  
            }
            $arr[] = array('order_id' => $value['order_id'],
                ...
                'order_status1' => $value['order_status1'],
                'order_status2' => $value['order_status2'],
                'order_status3' => $value['order_status3'],
    ...
    function drop_consignee($id) {
        ...
        if ($uid != $_SESSION['user_id']) {
            ...
            //判断当前地址是否是默认地址
             $sql1 = "SELECT address_id FROM " . $this->pre . "users WHERE user_id = '$uid'";
             $res1 = $this->row($sql1);
            $address_id = $res1['address_id'];
            if($address_id==$id){
              //取消默认值设置
            $sql2 = "UPDATE " . $this->pre .
                    "users SET address_id = 0 WHERE user_id = '$uid'";
    ...
    function save_consignee_default($address_id) {
        ...
        if($res!==false){
             $sql1 = "SELECT ua.*" .
                        " FROM " . $this->pre . "user_address AS ua left join " . $this->pre . "users AS u  on u.address_id=ua.address_id" .
                        " WHERE u.user_id=".$_SESSION['user_id'];
                $arr = $this->row($sql1);
            $_SESSION ['flow_consignee'] = $arr;    
        }
        return $res !== false;
    }
    /*
     * 取消默认配送地址
     */
    function cancel_address_default($address_id){
       /* 保存为用户的默认收货地址 */
        $sql = "UPDATE " . $this->pre .
                "users SET address_id = '$address_id' WHERE user_id = ".$_SESSION['user_id'];
        $res = $this->query($sql);
 ...
 * 新人注册赠送优惠券操作
 */
 public   function  add_user_bonus($userid){
        //检测是否已经赠送过新人优惠券
         $sql = 'SELECT count(bonus_id) as count FROM ' . $this->pre . 'user_bonus where bonus_type_id=1 AND user_id ='.$userid;
        $res = $this->row($sql);
        if($res['count']<=0){
               //  进行优惠券赠送操作
            $this->table = "user_bonus";
            $data['user_id'] = $userid;
            $data['bonus_type_id'] = 1;
 ...
 * 新人注册赠送优惠券操作++++++++++
 */
    public   function  new_add_user_bonus($userid){
        //检测是否已经赠送过新人优惠券
        $sql = 'SELECT count(bonus_id) as count FROM ' . $this->pre . 'user_bonus where bonus_type_id=1 AND user_id ='.$userid;
        $res = $this->row($sql);
        /*if($res['count']<=0){
            ...
            //$data['bonus_type_id'] = 1;
            //$data['use_start_date']=  strtotime(date("Y-m-d",gmtime())." 00:00:00");
            $use_start_date=  strtotime(date("Y-m-d",gmtime())." 00:00:00");
            //$data['use_end_date']=strtotime(date("Y-m-d",gmtime()+3600*24*15)." 23:23:59");
            $use_end_date =strtotime(date("Y-m-d",gmtime()+3600*24*15)." 23:23:59");
            $sql = 'insert into ' . $this->pre . 'user_bonus  (user_id,bonus_type_id,use_start_date,use_end_date) values ('.$userid.',1,'.$use_start_date.','.$use_end_date.')';
            for($i=1;$i<=5;$i++){
 ...
 * //判断用户是否领过新人专享优惠券
 */
 public function get_new_user_vouchers($userid){
     $sql = 'SELECT count(bonus_id) as count FROM ' . $this->pre . 'user_bonus where bonus_type_id=1 AND user_id ='.$userid;
     $res = $this->row($sql);
     if($res['count']<=0){
         $is_receive = 0;
     ...
     * 设置推荐人
     */
   /* public function  set_inviter_id($userid,$inviter_id){
        //获取推荐人信息
        $sql='SELECT parent_id FROM '.$this->pre.'users  where user_id='.$userid;
        $res=$this->row($sql);
        if($res['parent_id']==0 && $userid!=$inviter_id){
         //设置推荐人信息
         $sql2 = 'UPDATE ' . $this->pre . 'users SET parent_id = ' . $inviter_id . ' WHERE user_id = ' . $userid;
         $re=$this->query($sql2);
        }
    }*/
   ...
   */
     ...
     * 获取推荐人
     */
    public function get_inviter_id($userid){
        $sql='SELECT parent_id FROM '.$this->pre.'users  where user_id='.$userid;
        $res=$this->row($sql);
     ...
     * 获取邀请用户个数
     */
    public function  get_invitee_count($userid){
        $sql='SELECT count(user_id) as num FROM '.$this->pre.'users  where parent_id='.$userid;
        $res=$this->row($sql);
        return $res['num'];
    }
    /*
     * 获取邀请用户所得优惠券个数
     */
     public function  get_invitee_bonus_count($userid){
        $sql = 'SELECT count(bonus_id) as count FROM ' . $this->pre . 'user_bonus where bonus_type_id=2 AND user_id ='.$userid.' AND invitee_id!=0';
        $res=$this->row($sql);
        return $res['count'];
    }
     ...
     * 获取邀请用户所得优惠券个数
     */
    public function  get_all_bonus_count($userid){
        $sql = 'SELECT count(bonus_id) as count FROM ' . $this->pre . 'user_bonus where user_id ='.$userid;
        $res=$this->row($sql);
        return $res['count'];
    }
    /*
     * 邀请有礼优惠券赠送（被邀请人）
     */
    /*public function add_invitee_bonus($userid){
        //检测是否已经赠送过邀请优惠券
         $sql = 'SELECT count(bonus_id) as count FROM ' . $this->pre . 'user_bonus where bonus_type_id=2 AND user_id ='.$userid;
        $res = $this->row($sql);
        if($res['count']<=0){
               //  进行优惠券赠送操作
             $this->table = "user_bonus";
            ...
            $data['use_end_date']=strtotime(date("Y-m-d",gmtime()+3600*24*15)." 23:23:59");
            for($i=1;$i<=4;$i++){
              $this->model->table("user_bonus")->insert($data);   
            }
        }  
    }*/
    ...
    */
     ...
     * 邀请有礼优惠券赠送（邀请人）
     */
    public function  add_inviter_bonus($userid){
        //获取邀请人用户id
        $sql='SELECT parent_id FROM '.$this->pre.'users  where user_id='.$userid;
        $res=$this->row($sql);
        if($res['parent_id']!= 0){
         //检测是否已经赠送过邀请优惠券
         $sql2 = 'SELECT count(bonus_id) as count FROM ' . $this->pre . 'user_bonus where bonus_type_id=2 AND user_id ='.$res['parent_id'].' AND invitee_id='.$userid;
        $res2 = $this->row($sql2);
        if($res2['count']<=0){
               //  进行优惠券赠送操作
            $this->table = "user_bonus";
            $data['user_id'] = $res['parent_id'];
            $data['bonus_type_id'] = 2;
    ...
    * 获取我的余额
    */
    public function  get_user_money($userid){
        //获取邀请人用户id
        $sql='SELECT user_money FROM '.$this->pre.'users  where user_id='.$userid;
        $res=$this->row($sql);
        if(!empty($res)){
            return $res['user_money'];
        }
    }
