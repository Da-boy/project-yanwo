...
class IndexModel extends CommonModel {
    ...
    public function goods_list($type = 'best', $limit = 10, $start = 0) {
        ...
        $sql .= ' ORDER BY g.sort_order DESC limit ' . $start . ', ' . $limit;
     ...
     * @param  $start
     */
    public function new_goods_list($type = 'best', $limit = 10, $start = 0) {
        if ($type == 'new') {
            $type = 'g.is_new = 1';
        } else if ($type == 'hot') {
            $type = 'g.is_hot = 1';
        ...
        // 取出所有符合条件的商品数据，并将结果存入对应的推荐类型数组中
        $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price,g.little_label, ' . "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, " . "promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, g.goods_img, RAND() AS rnd " . 'FROM ' . $this->pre . 'goods AS g ' . "LEFT JOIN " . $this->pre . "member_price AS mp " . "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ";
        $sql .= ' WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND ' . $type;
        $sql .= ' ORDER BY g.sort_order DESC limit ' . $start . ', ' . $limit;
        ...
        foreach ($result as $key => $vo) {
            ...
            $goods[$key]['sales_count'] = model('GoodsBase')->get_sales_count($vo['goods_id']);
            $goods[$key]['sc'] = model('GoodsBase')->get_goods_collect($vo['goods_id']);
            $goods[$key]['little_label'] = $vo['little_label'];
            $goods[$key]['mysc'] = 0;
            //获取商品下属性
            $sql_attr = "select * from " . $this->pre . "goods_attr where goods_id = ".$vo['goods_id'];
            $attr_row = $this->query($sql_attr);
            if(!empty($attr_row)){
                /*$goods[$key]['shop_price'] = price_format($vo['shop_price']+$attr_row[0]['attr_price']);*/
                $goods[$key]['attr_row'] = $attr_row;
            }
            // 检查是否已经存在于用户的收藏夹
            if ($_SESSION ['user_id']) {
                // 用户自己有没有收藏过
