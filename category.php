...
if (!$smarty->is_cached($dwt_name.'.dwt', $cache_id))
{
	...
	{
		$on_top_goods = get_on_top_goods($cat_id,7);
		...
		$brands1 = $GLOBALS['db']->getAll($sql);
	
		foreach ($brands1 AS $key => $val)
		{
			$temp_key = $key + 1;
			$brands1[$temp_key]['brand_name'] = $val['brand_name'];
			$brands1[$temp_key]['url'] = build_uri('category', array('cid' => $cat_id, 'bid' => $val['brand_id'], 'price_min'=>$price_min, 'price_max'=> $price_max, 'filter_attr'=>$filter_attr_str), $cat['cat_name']);
	
			$brands1[$temp_key]['logo'] = !empty($val['brand_logo']) ? 'data/brandlogo/'.$val['brand_logo'] : '';
		}
		
		$brands1[0]['brand_name'] = $_LANG['all_attribute'];
		$brands1[0]['url'] = build_uri('category', array('cid' => $cat_id, 'bid' => 0, 'price_min'=>$price_min, 'price_max'=> $price_max, 'filter_attr'=>$filter_attr_str), $cat['cat_name']);
		$brands1[0]['selected'] = empty($brand) ? 1 : 0;
		
		unset($brands1[0]);
	
		$smarty->assign('brands1', $brands1);
	}
	else
	{
		$smarty->assign('cat_info',         $cat);
