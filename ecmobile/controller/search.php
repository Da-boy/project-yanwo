...
if ($sort_by == 'is_hot') {
    ...
    $sort = 'promote_price DESC, shop_price';
} elseif ($sort_by == 'price_asc') {
    $sort = 'promote_price ASC, shop_price';
    $order = 'ASC';
}
...
//   rc_log("children categorys ", json_encode($categories));
...
{
	//    var_dump(get_extension_goods($children));
	//    exit;
	...
	/* 获得商品列表 */
	$sql = 'SELECT g.goods_id,g.cat_id, g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
			"IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, g.promote_price, g.goods_type, " .
			'g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb, g.original_img, g.goods_img, g.seller_note ' .
			'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
			'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
			"ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
			"WHERE $where $ext ORDER BY $sort $order";
// 	rc_log("goods list 2 sql ", $sql);
...
// 		$arr[$row['goods_id']]['goods_img']        = get_image_path($row['goods_id'], $row['goods_img']);
// 		$arr[$row['goods_id']]['url']              = build_uri('goods', array('gid'=>$row['goods_id']), $row['goods_name']);
// 		$arr[$row['goods_id']]['count']         = ments_count_all_goods_index($row['goods_id']);
// 		$arr[$row['goods_id']]['cat_name']      = get_class_child($row['cat_id']);
