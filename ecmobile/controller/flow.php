...
switch ($tmp[0]) {
	case 'checkOrder':
		...
		//ECMOBAN--WeiDong 积分比率修改
		$out['integral_scale'] = $GLOBALS['_CFG']['integral_scale'];//积分兑换比率
	...
	case 'done':
	   ...
	   /* 判断是不是实体商品 */
	    ...
	    if ($order['pay_id'] > 0)
        {
            ...
            //如果是货到付款，状态设置为已确认。ECMoban--WeiDong
            if($payment['pay_code'] == 'cod') { $order['order_status'] = 1; }
