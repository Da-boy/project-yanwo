...
function get_order_detail($order_id, $user_id = 0)
{
    ...
    //切换手机订单的关联的支付方式 by carson start
    if($order['mobile_pay'] > 0){
        //查询手机版支付方式的配置参数
        $sql = "SELECT pay_config FROM " . $GLOBALS['ecs']->table('touch_payment') . " WHERE pay_id = '$order[pay_id]'";
        $touch_pay_config = $GLOBALS['db']->getOne($sql);
        $touch_store = unserialize($touch_pay_config);
        /* 取出已经设置属性的code */
        $touch_code_list = array();
        foreach ($touch_store as $key=>$value) {
            if($value['name'] == 'relate_pay'){
                $pc_pay_id = $value['value'];
            }
        }
        // 默认没有设置关联支付方式的
        if($pc_pay_id <= 0){
        ...
        if($pc_pay_id > 0 && $order['pay_status'] == PS_UNPAYED && $order['shipping_status'] == SS_UNSHIPPED && $order['goods_amount'] > 0){
            //查询电脑版支付方式
            $pc_payment_info = payment_info($pc_pay_id);
            $order['pay_id'] = $pc_payment_info['pay_id'];
            $order['pay_name'] = $pc_payment_info['pay_name'];
            $order_amount = $order['order_amount'] - $order['pay_fee'];
            $pay_fee = pay_fee($pc_pay_id, $order_amount);
            $order_amount += $pay_fee;
        
            $sql = "UPDATE " . $GLOBALS['ecs']->table('order_info') .
                   " SET pay_id='$pc_pay_id', pay_name='$pc_payment_info[pay_name]', pay_fee='$pay_fee', order_amount='$order_amount', `mobile_pay` = '0'".
                   " WHERE order_id = '$order_id'";
            $GLOBALS['db']->query($sql);
        }
    }
    //切换手机订单的关联的支付方式 by carson end
